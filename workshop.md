...
```text
Deploy this app to Azure App Service using Free Tier to be deployed in Region of Indonesia Central with the name of '<YOUR ASSIGNED USERNAME>-pomodoro-app'. Use this command 'az login --service-principal --username <AZURE_ASSIGNED_USERNAME> --password <AZURE_ASSIGNED_PASSWORD> --tenant <AZURE_ASSIGNED_TENANT>' and use the IndGHCPDeployment-RG resource group. Deploy this manually (zip) no need to use git or CI/CD.
```
